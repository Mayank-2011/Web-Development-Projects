...
def add_cafe():
    ...
    if form.validate_on_submit():
        new_cafe = [form.data]
        new_cafe[0].pop('submit')
        new_cafe[0].pop('csrf_token')
        new_cafe_list = []
        for key, value in new_cafe[0].items():
            new_cafe_list.append(value)
        with open('cafe-data.csv', newline='', encoding='utf-8', mode='a') as csv_file:
            new_row = csv.writer(csv_file)
            new_row.writerow(new_cafe_list)
